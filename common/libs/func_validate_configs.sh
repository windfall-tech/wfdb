  ...
  for config_param in $lfs_PARAM_LIST; do
    case $config_param in
      ...
      HOSTNAME)  echo -e "`eval echo $PARAM_VALS`"
        ...
        # Case of PKGMNGT: two files, packageManager.toml and packInstall.sh
        # must exist in $PKGMNGTDIR if PKGMNGT='y':
      PKGMNGT) echo -e "`eval echo $PARAM_VALS`"
               if [ "$PKGMNGT" = y ]; then
                 if [ ! -e "$PKGMNGTDIR/packageManager.toml" ]; then
                   write_pkg_and_die $PKGMNGTDIR/packageManager.toml does not exist
                 fi
                 if [ ! -e "$PKGMNGTDIR/packInstall.sh" ]; then
                   write_pkg_and_die $PKGMNGTDIR/packInstall.sh does not exist
                 fi
                 if [ ! -s "$PKGMNGTDIR/packageManager.toml" ]; then
                   write_pkg_and_die $PKGMNGTDIR/packageManager.toml has zero size
                 fi
                 if [ ! -s "$PKGMNGTDIR/packInstall.sh" ]; then
                   write_pkg_and_die $PKGMNGTDIR/packInstall.sh has zero size
                 fi
                 if [ ! -r "$PKGMNGTDIR/packageManager.toml" ]; then
                   write_pkg_and_die $PKGMNGTDIR/packageManager.toml is not readable
                 fi
